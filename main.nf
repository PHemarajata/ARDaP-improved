...
// Add an overrideable output directory parameter (can be set with --outdir)
...
if (params.assemblies) {
  ...
  // helper scripts used by this process are declared as inputs so Nextflow
  // stages them into the task work dir and includes them in the process hash
  file run_resfinder_sh from file("${baseDir}/bin/Run_resfinder.sh")
...
*/
  ...
  file ref_index from ref_index_ch
  file run_resfinder_sh from file("${baseDir}/bin/Run_resfinder.sh")
  // Masked_alignment.sh is called conditionally; stage it to the work dir
  file masked_alignment_sh from file("${baseDir}/bin/Masked_alignment.sh")
...
if (params.mixtures) {
  ...
  process MixtureSummariesSQL {
    ...
    process VariantCalling {
      ...
      file reference_fai from ref_fai_ch1
      file reference_dict from ref_dict_ch1
  // stage the variant calling helper script so it's included in the process hash
  file variant_calling_sh from file("${baseDir}/bin/VariantCalling.sh")
...
*/
 if (params.mixtures) {
  ...
  // stage SQL and reporting helper scripts
  file sql_snps_indel_mix from file("${baseDir}/bin/SQL_queries_SNP_indel_mix.sh")
  file sql_deldup_mix from file("${baseDir}/bin/SQL_queries_DelDupMix.sh")
  file abr_reports_mix from file("${baseDir}/bin/AbR_reports_mix.sh")
  ...
  process SqlSnpsIndels {
    ...
    file resistance_db from resistance_database_file
  // stage SQL and reporting helper scripts
  file sql_snps_indel from file("${baseDir}/bin/SQL_queries_SNP_indel.sh")
  file sql_deldup from file("${baseDir}/bin/SQL_queries_DelDup.sh")
  file abr_reports from file("${baseDir}/bin/AbR_reports.sh")
...
process R_report {
  ...
  // stage the report html helper so changes trigger re-run of reporting only
  file report_html_sh from file("${baseDir}/bin/Report_html.sh")
  ...
  script:
  """
  bash Report_html.sh ${id} ${species}
  """
}
...
if (params.phylogeny) {
  process VariantCallingGVCF {
    ...
    label "gatk"
    tag { "$id" }
    publishDir "${params.outdir}/Variants/GVCFs", mode: 'copy', overwrite: false, pattern: '*.gvcf'
  ...
  process Master_vcf {
   ...
   // tag { "$id" }
    ...
    file reference_fai from ref_fai_ch1
    file reference_dict from ref_dict_ch1
  file master_vcf_sh from file("${baseDir}/bin/Master_vcf.sh")
  ...
  process snp_matrix {
    ...
    file snp_matrix_sh from file("${baseDir}/bin/SNP_matrix.sh")
...
Done! Result files are in --> ${params.outdir}

  Antibiotic resistance reports are in --> ${params.outdir}/AbR_reports

  If further analysis is required, bam alignments are in --> ${params.outdir}/bams

  Phylogenetic tree and annotated merged variants are in --> ${params.outdir}/Phylogeny_and_annotation

  Individual variant files are in --> ${params.outdir}/Variants/VCFs
