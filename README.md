...
2) Install an ARDaP conda environment

```
Use the full environment if possible.
```
conda env create --name ardap-updated -f ./ardap/env_full.yaml
```
Load the conda environment

```
conda activate ardap-updated
```

The pipeline itself should ALWAYS be run by calling the main.nf. 
...
Optional Parameter: \
  ...
  `$ nextflow run main.nf --species Burkholderia_pseudomallei --mixtures`
  or
  ...
  `$ nextflow run main.nf --species Pseudomonas_aeruginosa --size 0 --mixtures`
  or
  ...
  `$ nextflow run main.nf --species Pseudomonas_aeruginosa --phylogeny`
  or
  ...
  For example: \
  `nextflow run main.nf --species Pseudomonas_aeruginosa`
  or
...
```
`nextflow run main.nf --fastq "*_{1,2}_sequence.fq.gz"`
or
...
NB. A complete, closed archetypal reference genome should be used where possible, and this genome must be present in the SnpEff database. Run the following command to identify the available SnpEff databases for your microbe of interest: e.g. `conda activate ardap-updated && snpEff databases | grep 'Burkholderia_pseudomallei'`

## snpEff (v5+) — isolated environment

ARDaP requires snpEff v5 (or newer) to read the bundled `snpEffectPredictor.bin` databases. If you encounter compatibility errors ("Database version X vs Program version Y"), create the dedicated snpEff environment shipped with this repository:

```
conda env create -f envs/snpeff.yml
```

This creates a small environment named `snpeff_env` (the name can be changed inside `envs/snpeff.yml`). ARDaP has been updated so the main snpEff-using helper scripts invoke snpEff via a wrapper variable `SNPEFF_CMD` which defaults to:

```
SNPEFF_CMD="conda run -n snpeff_env snpEff"
```

You can override `SNPEFF_CMD` in your shell or set it in your job submission script if you prefer to use a container or a different snpEff installation. Example verification of the snpEff env:

```
conda run -n snpeff_env snpEff -version
```

If you'd rather use containers, create a Docker/Singularity image with snpEff v5 and set `process.container` in `nextflow.config` for `snpeff`-labelled processes.

The repository already includes pre-built `snpEffectPredictor.bin` files under `resources/snpeff/` for several genomes; if you add new genomes, generate the database with snpEff v5 and place it under `resources/snpeff/<genome>/snpEffectPredictor.bin`.

## Nextflow profiles for common hardware

Two convenience profiles have been added to `nextflow.config` for typical local hardware:

- `workstation` — tuned for a single workstation (~22 cores, 64 GB RAM)
- `dgx` — tuned for an NVIDIA DGX Station A100 (~128 cores, 512 GB RAM)

Use them like:

```
nextflow run main.nf -profile workstation --fastq "*_R{1,2}*.fastq.gz" --species Burkholderia_pseudomallei
```

These profiles adjust per-step CPU and memory allocations. If you have different hardware, edit `nextflow.config` or create a new profile.
