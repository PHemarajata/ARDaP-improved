...
cat << _EOF_ >  ${seq}.Variant_ignore_Q.txt  
SELECT
...
sqlite3 "$RESISTANCE_DB" < ${seq}.Variant_ignore_Q.txt >> ${seq}.Variant_ignore.txt;

sed -i 's/|/ /g' ${seq}.Variant_ignore.txt 
...
for (( i=1; i<"$UPREG_COUNT_MIX"; i++ )); do 
	sqlite3 "$RESISTANCE_DB" "${SQL_upregulation_report_mix[$i]}" | tee -a ${seq}.output.temp ${seq}.AbR_output_del_dup_mix.txt
	if [ -s ${seq}.output.temp ]; then
		...
		echo "$mixture_percent" | paste ${seq}.output.temp - >> ${seq}.AbR_output_del_dup_mix.txt
		rm ${seq}.output.temp
	fi
done
echo "done"
...
for (( i=1; i<"$LOSS_COUNT_MIX"; i++ )); do 
	sqlite3 "$RESISTANCE_DB" "${SQL_loss_report_mix[$i]}" | tee -a ${seq}.output.temp ${seq}.AbR_output_del_dup_mix.txt
	if [ -s ${seq}.output.temp ]; then
...
for (( i=1; i<"$LOSS_FUNC_COUNT"; i++ )); do
	sqlite3 "$RESISTANCE_DB" "${SQL_loss_func[$i]}" | tee -a ${seq}.output.temp ${seq}.AbR_output_del_dup_mix.txt
	if [ -s ${seq}.output.temp ]; then
