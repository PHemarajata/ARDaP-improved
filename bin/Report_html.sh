...
# Usage: Report_html.sh <sample_id> <species>
id=$1
speciestmp=$2
species=${speciestmp/_/ }

# Grab the metadata row for this sample id (fallback to last line if not found)
meta_line=$(awk -F"," -v id="$id" '$1==id {print; exit}' patientMetaData.csv)
if [ -z "$meta_line" ]; then
  meta_line=$(tail -n1 patientMetaData.csv)
fi

# Parse metadata fields from the selected line
ID=$(echo "$meta_line" | awk -F"," '{ print $1 }')
Barcode=$(echo "$meta_line" | awk -F"," '{ print $2 }')
LName=$(echo "$meta_line" | awk -F"," '{ print $3 }')
FName=$(echo "$meta_line" | awk -F"," '{ print $4 }')
DOB=$(echo "$meta_line" | awk -F"," '{ print $5 }')
Location=$(echo "$meta_line" | awk -F"," '{ print $6 }')
sampType=$(echo "$meta_line" | awk -F"," '{ print $7 }')
sampID=$(echo "$meta_line" | awk -F"," '{ print $8 }')
sampDate=$(echo "$meta_line" | awk -F"," '{ print $9 }')
sampSource=$(echo "$meta_line" | awk -F"," '{ print $10 }')
sampSeq=$(echo "$meta_line" | awk -F"," '{ print $11 }')
reportLab=$(echo "$meta_line" | awk -F"," '{ print $12 }')
reportDate=$(echo "$meta_line" | awk -F"," '{ print $13 }')
comments=$(echo "$meta_line" | awk -F"," '{ print $14 }')
organism=$(echo "$meta_line" | awk -F"," '{ print $15 }')
requestor=$(echo "$meta_line" | awk -F"," '{ print $16 }')
requestorContact=$(echo "$meta_line" | awk -F"," '{ print $17 }')
lineageNum=$(echo "$meta_line" | awk -F"," '{ print $18 }')
lineageName=$(echo "$meta_line" | awk -F"," '{ print $19 }')
...
# echo to file and then change last comma to . (use sample-scoped temp)
echo "${resistant_list[@]}" > ${id}_list_tmp
sed -i 's/,$/./' ${id}_list_tmp
resistant_drugs=$(cat ${id}_list_tmp)
...
readarray -t f_line_level < <( awk -F"," '$2 ~ /[Ff]irst-line/ ' patientDrugSusceptibilityData.csv | awk -F"," '{print $5}')
readarray -t s_line_level < <( awk -F"," '$2 ~ /[Ss]econd-line/ ' patientDrugSusceptibilityData.csv | awk -F"," '{print $5}')

readarray -t f_line_mech < <( awk -F"," '$2 ~ /[Ff]irst-line/ ' patientDrugSusceptibilityData.csv | awk -F"," '{print $6}')
readarray -t s_line_mech < <( awk -F"," '$2 ~ /[Ss]econd-line/ ' patientDrugSusceptibilityData.csv | awk -F"," '{print $6}')
...
if [ "$noTertiary" -gt 0 ]; then
  ...
  readarray -t t_line_level < <( awk -F"," '$2 ~ /[Tt]ertiary/ ' patientDrugSusceptibilityData.csv | awk -F"," '{print $5}')
  readarray -t t_line_mech < <( awk -F"," '$2 ~ /[Tt]ertiary/ ' patientDrugSusceptibilityData.csv | awk -F"," '{print $6}')
...
if [ "$noIntrinsic" -gt 0 ]; then
  ...
  readarray -t i_line_level < <( awk -F"," '$2 ~ /[Ii]ntrinsic/ ' patientDrugSusceptibilityData.csv | awk -F"," '{print $5}')
  readarray -t i_line_mech < <( awk -F"," '$2 ~ /[Ii]ntrinsic/ ' patientDrugSusceptibilityData.csv | awk -F"," '{print $6}')
...
awk -F"|" '$4 ~ /[Nn]one/ ' ${id}.AbR_output.final.txt > ${id}.natural_variation.txt
awk -F"|" '$4 !~ /[Nn]one/ ' ${id}.AbR_output.final.txt > ${id}.resistance_determinants.txt

sed -i 's/>/\&gt;/g' ${id}.resistance_determinants.txt
sed -i 's/</\&lt;/g' ${id}.resistance_determinants.txt
sed -i 's/≥/\&#8925;/g' ${id}.resistance_determinants.txt
sed -i 's/µ/\&#181;/g' ${id}.resistance_determinants.txt
sed -i 's/≤/\&#8804;/g' ${id}.resistance_determinants.txt
sed -i 's/^/<tr><td WIDTH="100%">/g' ${id}.resistance_determinants.txt
sed -i 's/$/<\/td><\/tr>/g' ${id}.resistance_determinants.txt


sed -i 's/>/\&gt;/g' ${id}.natural_variation.txt
sed -i 's/</\&lt;/g' ${id}.natural_variation.txt
sed -i 's/≥/\&#8925;/g' ${id}.natural_variation.txt
sed -i 's/µ/\&#181;/g' ${id}.natural_variation.txt
sed -i 's/≤/\&#8804;/g' ${id}.natural_variation.txt
sed -i 's/^/<tr><td WIDTH="100%">/g' ${id}.natural_variation.txt
sed -i 's/$/<\/td><\/tr>/g' ${id}.natural_variation.txt

abr_report=$(cat ${id}.resistance_determinants.txt)
natural_variation=$(cat ${id}.natural_variation.txt)

#Format for speciation report
awk -F"|" -v f="speciation" 'BEGIN{IGNORECASE=1} $4~ f' ${id}.AbR_output.final.txt > ${id}.speciation.txt
sed -i 's/^/<tr><td WIDTH="100%">/g' ${id}.speciation.txt
sed -i 's/$/<\/td><\/tr>/g' ${id}.speciation.txt
speciation=$(cat ${id}.speciation.txt)
...
<thead>
<tr>
<th colspan="5">Drug Susceptibility</th>
</tr>
</thead>
<tbody>
<tr>
...
<tr>
<td WIDTH="20%"><strong>Drug class</strong></td>
<td WIDTH="20%"><strong>Interpretation</strong></td>
<td WIDTH="20%"><strong>Drug</strong></td>
<td WIDTH="20%"><strong>Level (0-49=sensitive; 50-99=intermediate; >99=resistant) </strong></td>
<td WIDTH="20%"><strong>Resistance determinant</strong></td>
</tr>
$first_line_table
${f_line_table_more[@]}
$second_line_table
...
<thead>
<tr>
<th colspan="5">Extended/non-clinical drug susceptibility</th>
</tr>
</thead>
<tbody>
<tr>
<td WIDTH="20%">Drug class</td>
<td WIDTH="20%">Interpretation</td>
<td WIDTH="20%">Drug</td>
<td WIDTH="20%">Level</td>
<td WIDTH="20%">Resistance determinant</td>
</tr>
$tertiary_line_table
${t_line_table_more[@]}
</tbody>
...
<thead>
<tr>
<th colspan="5">Intrinsic drug resistance/unusual drug susceptibility</th>
</tr>
</thead>
<tbody>
<tr>
...
<td WIDTH="20%">Drug</td>
<td WIDTH="20%">Level</td>
<td WIDTH="20%">Mechanism of sensitivity</td>
</tr>
...
<TR>
  ...
  <footer class="text-muted small mb-4">
    Generated by ARDaP report generator
  </footer>
