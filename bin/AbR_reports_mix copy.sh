...
# Robust CSV generator (legacy sed-based edits replaced by explicit generator)
...
while IFS=, read -r drug_class drug_name abbrev; do
    ...
    matches=$(awk -F"|" -v a="$abbrev" 'BEGIN{IGNORECASE=1} $4 ~ a"r" || $4 ~ a"i" || $4 ~ a"s" { print $0 }' AbR_output.final.txt)
